...
local function IsValidPreferenceName(Name)
	...
	-- Aceita qualquer nome não vazio após normalização (validação real acontece no mundo)
...
local function LoadPreferencesFromConfig()
	...
	if type(raw) ~= "table" or #raw == 0 then
		PreferencesList = table.clone(DefaultPreferences)
		RebuildESPPriority()
		return
	end
	local cleaned = {}
...
-- Carrega preferências logo no início
