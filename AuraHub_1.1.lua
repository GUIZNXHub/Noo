--========================================================
-- SNOO HUB V1.1
-- Preferências dinâmicas + Linha mais visível + Nome em TODOS os Brainrots
-- Caixa roxa/preta semi-transparente que sempre olha pro player
...
-- =========================================================
-- PREFERÊNCIAS (lista dinâmica de prioridade)
-- =========================================================
...
local PreferencesList = {} -- array ordenado: [1] = maior prioridade
local ESPPriority = {}     -- mapa normalizado -> prioridade numérica (1 = melhor)
local PreferencesUI = {}   -- referências de UI para atualização

-- Polyfills (table.clone / table.clear nem sempre existem em executors)
...
local function TableCloneArray(t)
	local copy = {}
...
local function RebuildESPPriority()
	TableClear(ESPPriority)
	for i, name in ipairs(PreferencesList) do
		local norm = ESPNormalize(name)
		if norm ~= "" then
			ESPPriority[norm] = i
		end
	end
...
local function IsValidPreferenceName(Name)
	if type(Name) ~= "string" then return false end
	local trimmed = Name:match("^%s*(.-)%s*$") or ""
	if trimmed == "" or #trimmed < 2 then return false end
	local norm = ESPNormalize(trimmed)
	return norm ~= ""
end

local function PreferenceExistsInList(Name)
	local norm = ESPNormalize(Name)
	for _, existing in ipairs(PreferencesList) do
		if ESPNormalize(existing) == norm then
			return true
		end
	end
	return false
...
local function LoadPreferencesFromConfig()
	local raw = LoadedConfig.Preferences
	if type(raw) ~= "table" or #raw == 0 then
		PreferencesList = TableCloneArray(DefaultPreferences)
		RebuildESPPriority()
		return
	end
	local cleaned = {}
	...
	if #cleaned == 0 then
		...
		PreferencesList = cleaned
	end
	RebuildESPPriority()
end

local function GetPreferencesForSave()
	local copy = {}
	for _, name in ipairs(PreferencesList) do
		table.insert(copy, name)
	end
	return copy
end

-- Carrega preferências logo no início (protegido)
local loadOk, loadErr = pcall(LoadPreferencesFromConfig)
if not loadOk then
	warn("[SnooHub] Falha ao carregar preferências:", loadErr)
	PreferencesList = TableCloneArray(DefaultPreferences)
	RebuildESPPriority()
end
...
-- limpa gui antiga caso exista com outro nome
local Existing2 = PlayerGui:FindFirstChild("Snoo Hub")
...
print("[SnooHub] GUI criada, montando interface...")
...
local function CreateTab(Name, Text)
	...
	Button.Size = UDim2.fromOffset(68, 30)
...
CreateTab("ESP", "ESP")
CreateTab("Prefs", "PREFS")
CreateTab("Config", "CONFIG")

SelectPage("Movement")
for _, TabName in ipairs({"Movement", "Utility", "Jump", "Credits", "ESP", "Prefs", "Config"}) do
...
-- ==================== SPAM TP ====================
local SpamTPV1Enabled, SpamTPV2Enabled = false, false
local SpamTPV1Connection, SpamTPV2Connection = nil, nil
...
local function ApplyFPSBoost()
	...
	pcall(function()
		...
		if PerformanceMode == "Extreme" then
			Lighting.FogEnd = math.min(Lighting.FogEnd, 2500)
		elseif PerformanceMode == "Ultra" then
			Lighting.FogEnd = math.min(Lighting.FogEnd, 5000)
		end
	end)
	pcall(function()
...
for _, Btn in ipairs({SaveTPButton, TPButtonSmall, TPBestButton, TravelButtonSmall, SpamTPV1ButtonSmall, SpamTPV2ButtonSmall}) do
...
SaveConfig = function()
	...
	local Data = {
		...
		Preferences = GetPreferencesForSave(),
	}
...
-- =========================================================
-- ESP ULTRA V1.0 (usa Preferências dinâmicas)
-- =========================================================

local function ESPPriorityOf(Name)
	return ESPPriority[ESPNormalize(Name)]
end
...
local function ESPGetBrainrotName(Object)
	...
	for _, value in ipairs(candidates) do
		local prio = ESPPriorityOf(value)
...
-- ==================== NOME EM TODOS OS BRAINROTS ====================
local ActiveNameTags = {} -- [Object] = BillboardGui
...
local function UpdateAllNameTags()
	...
	for obj, data in pairs(found) do
		...
		if existing and existing.Parent then
			if existing.Adornee ~= data.Part then
				existing.Adornee = data.Part
			end
		else
			local bill = CreateNameTag(data.Part, data.Name)
			if bill then
				ActiveNameTags[obj] = bill
			end
		end
	end
end

-- ==================== BEST TARGET + LINHA MAIS VISÍVEL ====================
...
-- Página ESP
SelectPage("ESP")
Section("ESP ULTRA V1.0")
Description("Linha mais visível + Nome em TODOS os Brainrots da lista")
Description("Caixa roxa/preta semi-transparente que sempre olha pro player")
...
-- =========================================================
-- PREFERÊNCIAS UI (aba PREFS)
-- =========================================================

SelectPage("Prefs")
Section("PREFERÊNCIAS")
Description("Posição 1 = maior prioridade. Alterações salvas automaticamente.")
Description("Use ▲ ▼ para reordenar • Remover • Adicionar novo nome")

local PrefsStatusLabel = Label("Lista carregada (" .. #PreferencesList .. " itens)")
PrefsStatusLabel.TextColor3 = Color3.fromRGB(154, 158, 177)

-- Container da lista (ScrollingFrame interno)
local PrefsListContainer = Instance.new("Frame")
PrefsListContainer.Name = "PrefsListContainer"
PrefsListContainer.Size = UDim2.new(1, -8, 0, 220)
PrefsListContainer.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
PrefsListContainer.BorderSizePixel = 0
PrefsListContainer.ClipsDescendants = true
PrefsListContainer.ZIndex = 4
PrefsListContainer.Parent = CurrentPage
Instance.new("UICorner", PrefsListContainer).CornerRadius = UDim.new(0, 10)
local PrefsListStroke = Instance.new("UIStroke", PrefsListContainer)
PrefsListStroke.Color = Color3.fromRGB(40, 40, 48)

local PrefsScroll = Instance.new("ScrollingFrame", PrefsListContainer)
PrefsScroll.Name = "PrefsScroll"
PrefsScroll.Size = UDim2.new(1, -6, 1, -6)
PrefsScroll.Position = UDim2.fromOffset(3, 3)
PrefsScroll.BackgroundTransparency = 1
PrefsScroll.BorderSizePixel = 0
PrefsScroll.ScrollBarThickness = 3
PrefsScroll.ScrollBarImageColor3 = Color3.fromRGB(155, 70, 255)
PrefsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
PrefsScroll.ZIndex = 5

local PrefsScrollLayout = Instance.new("UIListLayout", PrefsScroll)
PrefsScrollLayout.Padding = UDim.new(0, 3)
PrefsScrollLayout.SortOrder = Enum.SortOrder.LayoutOrder
PrefsScrollLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

PrefsScrollLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
	PrefsScroll.CanvasSize = UDim2.fromOffset(0, PrefsScrollLayout.AbsoluteContentSize.Y + 6)
end)

local SelectedPrefIndex = nil -- índice selecionado na lista

local function SetPrefsStatus(text, isError)
	PrefsStatusLabel.Text = text
	PrefsStatusLabel.TextColor3 = isError and Color3.fromRGB(255, 90, 90) or Color3.fromRGB(154, 158, 177)
end

local function RefreshPreferencesUI()
	-- limpa rows antigas
	for _, child in ipairs(PrefsScroll:GetChildren()) do
		if child:IsA("Frame") then child:Destroy() end
	end
	SelectedPrefIndex = nil
	...
	for i, name in ipairs(PreferencesList) do
		...
		row.ZIndex = 6
		row.LayoutOrder = i
		row.Parent = PrefsScroll
		Instance.new("UICorner", row).CornerRadius = UDim.new(0, 8)

		local posLabel = Instance.new("TextLabel", row)
		posLabel.Size = UDim2.fromOffset(28, 28)
		posLabel.Position = UDim2.fromOffset(4, 0)
		posLabel.BackgroundTransparency = 1
		posLabel.Text = tostring(i)
		posLabel.TextColor3 = Color3.fromRGB(190, 90, 255)
		posLabel.TextSize = 11
		posLabel.Font = Enum.Font.GothamBlack
		posLabel.ZIndex = 7
		...
		upBtn.Size = UDim2.fromOffset(26, 22)
		upBtn.Position = UDim2.new(1, -92, 0.5, -11)
		upBtn.BackgroundColor3 = Color3.fromRGB(32, 28, 42)
		upBtn.BorderSizePixel = 0
		upBtn.Text = "▲"
		upBtn.TextColor3 = Color3.fromRGB(200, 180, 255)
		upBtn.TextSize = 10
		...
		downBtn.Size = UDim2.fromOffset(26, 22)
		downBtn.Position = UDim2.new(1, -62, 0.5, -11)
		downBtn.BackgroundColor3 = Color3.fromRGB(32, 28, 42)
		downBtn.BorderSizePixel = 0
		downBtn.Text = "▼"
		downBtn.TextColor3 = Color3.fromRGB(200, 180, 255)
		downBtn.TextSize = 10
		...
		remBtn.Size = UDim2.fromOffset(26, 22)
		remBtn.Position = UDim2.new(1, -32, 0.5, -11)
		remBtn.BackgroundColor3 = Color3.fromRGB(50, 20, 30)
		remBtn.BorderSizePixel = 0
		remBtn.Text = "X"
		remBtn.TextColor3 = Color3.fromRGB(255, 120, 120)
		remBtn.TextSize = 11
		...
		local function SelectThis()
			SelectedPrefIndex = i
			for _, r in ipairs(PrefsScroll:GetChildren()) do
				if r:IsA("Frame") then
					r.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
				end
			end
			row.BackgroundColor3 = Color3.fromRGB(55, 30, 85)
		end
		...
		upBtn.MouseButton1Click:Connect(function()
			...
			PreferencesList[i], PreferencesList[i - 1] = PreferencesList[i - 1], PreferencesList[i]
			RebuildESPPriority()
			ESPBestCache = nil
			if SaveConfig then SaveConfig() end
			SetPrefsStatus("Prioridade atualizada • salvo")
			RefreshPreferencesUI()
		end)

		downBtn.MouseButton1Click:Connect(function()
			if i >= #PreferencesList then return end
			PreferencesList[i], PreferencesList[i + 1] = PreferencesList[i + 1], PreferencesList[i]
			RebuildESPPriority()
			ESPBestCache = nil
			if SaveConfig then SaveConfig() end
		...
		remBtn.MouseButton1Click:Connect(function()
			table.remove(PreferencesList, i)
			RebuildESPPriority()
			ESPBestCache = nil
			if SaveConfig then SaveConfig() end
			SetPrefsStatus("Removido: " .. name)
			RefreshPreferencesUI()
		end)
	end

	PrefsStatusLabel.Text = "Lista (" .. #PreferencesList .. " itens) • 1 = maior prioridade"
	PrefsStatusLabel.TextColor3 = Color3.fromRGB(154, 158, 177)
end

-- Adicionar nova preferência
Label("ADICIONAR PREFERÊNCIA")
local AddPrefBox = TextBox("", "Digite o nome exato do brainrot")
local AddPrefButton = Button("  ADICIONAR À LISTA")
local RestoreDefaultButton = Button("  RESTAURAR PREFERÊNCIAS PADRÃO")

AddPrefButton.MouseButton1Click:Connect(function()
	local raw = AddPrefBox.Text or ""
	local trimmed = raw:match("^%s*(.-)%s*$") or ""
	if trimmed == "" then
		SetPrefsStatus("Digite um nome válido", true)
		return
	end
	if not IsValidPreferenceName(trimmed) then
		SetPrefsStatus("Nome inválido", true)
		return
	end
	if PreferenceExistsInList(trimmed) then
		SetPrefsStatus("Já existe na lista (sem duplicatas)", true)
		return
	end

	-- Validação: verifica se o nome corresponde a algum item já conhecido na lista padrão
	-- ou se existe no workspace (Plots). Aceita se bater com a lista padrão OU se for
	-- encontrado no mundo no momento.
	local norm = ESPNormalize(trimmed)
	local isKnownDefault = false
	for _, def in ipairs(DefaultPreferences) do
		if ESPNormalize(def) == norm then
			isKnownDefault = true
			trimmed = def -- usa a grafia oficial
			break
		end
	end

	local foundInWorld = false
	if not isKnownDefault then
		local Plots = workspace:FindFirstChild("Plots")
		if Plots then
			for _, obj in ipairs(Plots:GetDescendants()) do
				local name, _ = ESPGetBrainrotName(obj)
				if name and ESPNormalize(name) == norm then
					foundInWorld = true
					trimmed = name -- usa a grafia encontrada
					break
				end
				-- também checa o Name bruto
				if ESPNormalize(obj.Name) == norm then
					foundInWorld = true
					trimmed = obj.Name
					break
				end
	...
	if not isKnownDefault and not foundInWorld then
		SetPrefsStatus("Item não encontrado: \"" .. trimmed .. "\"", true)
		return
	end

	table.insert(PreferencesList, trimmed)
	RebuildESPPriority()
	ESPBestCache = nil
	if SaveConfig then SaveConfig() end
	AddPrefBox.Text = ""
	SetPrefsStatus("Adicionado: " .. trimmed)
	RefreshPreferencesUI()
end)

RestoreDefaultButton.MouseButton1Click:Connect(function()
	PreferencesList = TableCloneArray(DefaultPreferences)
	RebuildESPPriority()
	ESPBestCache = nil
	if SaveConfig then SaveConfig() end
	SetPrefsStatus("Preferências padrão restauradas")
	RefreshPreferencesUI()
end)

-- monta a UI inicial das preferências
RefreshPreferencesUI()
...
SelectPage("Config")
Section("PERFORMANCE")
Description("Modo Ultra reduz sombras, partículas, efeitos e qualidade para priorizar FPS.")
...
PerformanceButton.MouseButton1Click:Connect(function()
	...
	local Index = TableFind(Modes, PerformanceMode) or 2
...
local LanguageHint = Description("pt-BR • pt-PT • en-US • es-ES")
...
LanguageButton.MouseButton1Click:Connect(function()
	local Index = TableFind(LanguageOrder, Language) or 1
...
if PerformanceButton then PerformanceButton.Text = "  FPS BOOST " .. PerformanceMode:upper() end
if FPSApplyButton then FPSApplyButton.Text = FPSBoostEnabled and "  FPS BOOST ATIVO" or "  ATIVAR FPS BOOST AGORA" end
AutoStealButton.Text = "  AUTO STEAL • " .. (AutoStealEnabled and "ON" or "OFF")
InfiniteButton.Text = "  INFINITY JUMP • " .. (InfiniteJumpEnabled and "ON" or "OFF")
SpamTPV1Button.Text = "  SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF")
SpamTPV2Button.Text = "  SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF")
...
print("Snoo Hub V1.1 carregado com sucesso! (Preferências dinâmicas)")
